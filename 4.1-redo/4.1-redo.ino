...
// this checks if switch is doing manual override
...
void loop()
{
  ...
  if (switchTriggered)
  {
    ...
    if (switchStatus())
    {
      manualOverride = true;
      allLightsOn();
      Serial.println("switch is turned ON, the manual override active hence both lights ON");
    }
    else
    {
      manualOverride = false;
      allLightsOff();
      Serial.println("switch is turned OFF, override has stopped. now both lights OFF");
    }
  }

  //timing for turning lights off
  if (lightsOn && !manualOverride)
  {
...
// manual override turns both lights on
void allLightsOn()
{
  porchLightOn();
  hallwayLightOn();
...
// manual override turns both lights off
void allLightsOff()
{
  porchLightOff();
  hallwayLightOff();
